...
const orderCurrency = (o: { shipping_type?: string }) =>
  o.shipping_type === "domestic" ? "TRY" : "EUR";
...
const AdminDashboard = () => {
  ...
  // Access is gated by <ProtectedRoute requireAdmin> — the user here always has a
  // real Supabase session with the 'admin' role, so RLS allows reading/updating all rows.
  const fetchOrders = async () => {
    try {
      ...
      setOrders((data as ShippingRequest[]) || []);
    } catch (error) {
      console.error("Error fetching orders:", error);
      toast.error("Siparişler yüklenemedi");
    } finally {
  ...
  const updateStatus = async (id: string, newStatus: OrderStatus) => {
    ...
    if (error || !data || data.length === 0) {
      ...
      setOrders(previous); // revert on failure (incl. RLS 0-row silent failure)
      return;
    }
  ...
  const openWhatsApp = (order: ShippingRequest) => {
    const message = encodeURIComponent(
      ...
      `• Tahmini Fiyat: ${formatMoney(order.estimated_min_price, orderCurrency(order))} - ${formatMoney(order.estimated_max_price, orderCurrency(order))}\n\n` +
                  ...
                  {displayOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="whitespace-nowrap font-medium">
                        {formatMoney(order.estimated_min_price, orderCurrency(order))}
                        {" - "}
                        {formatMoney(order.estimated_max_price, orderCurrency(order))}
                      </TableCell>
                      <TableCell>
